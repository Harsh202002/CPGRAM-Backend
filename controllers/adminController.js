...
exports.createOfficer = async (req, res, next) => {
  try {
    const { fullName, email, phoneNumber, gender, dob, password, department } =
      req.body;
  ...
  try {
    ...
    const usersWithGrievances = await Promise.all(
      users.map(async (user) => {
        const grievances = await Grievance.find({ user: user._id }).populate(
          "assignedTo",
          "fullName email phoneNumber"
        );
...
exports.getAllOfficers = async (req, res) => {
  try {
    const officers = await User.find(
      { role: "officer" },
      "fullName email phoneNumber gender department"
    );
    res.status(200).json({
      success: true,
      data: officers,
...
exports.getAllLeadOfficers = async (req, res) => {
  try {
    const leadOfficers = await User.find(
      { role: "lead_officer" },
      "fullName email phoneNumber gender department"
    );
    res.status(200).json({
